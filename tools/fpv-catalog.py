...
SHOWROOM_KINDS = (
    "frame", "motor", "propeller", "battery", "camera", "controller", "landing_gear", "attachment",
)
...
def prepare(args: argparse.Namespace) -> int:
    ...
    run([str(python), "-m", "pip", "install", "-e", f"{ROOT}[showroom]"], cwd=ROOT)
...
def doctor(args: argparse.Namespace) -> int:
    try:
        ...
        raise CatalogToolError("catalog showroom dependencies are incomplete; run prepare first") from exc

    catalogs = load_catalogs(catalog_paths(args))
    groups = (catalogs.frames, catalogs.motors, catalogs.propellers, catalogs.batteries, catalogs.cameras, catalogs.controllers, catalogs.landing_gears, catalogs.attachments)
...
def open_viewer(args: argparse.Namespace) -> int:
    from fpv_drone_generator.catalog import load_catalogs
    from fpv_drone_generator.showroom import generate_catalog_showroom, open_catalog_showroom
    catalogs = load_catalogs(catalog_paths(args))
    output = args.output.resolve() if args.output is not None else args.work_dir.resolve() / "catalog-showroom.xml"
    generate_catalog_showroom(catalogs, output, kind=args.kind, item_id=args.item_id)
    selection = "all components" if args.kind is None else (args.kind if args.item_id is None else f"{args.kind}/{args.item_id}")
...
def export_glb(args: argparse.Namespace) -> int:
    ...
    from fpv_drone_generator.catalog_glb import export_catalog_glb
    catalogs = load_catalogs(catalog_paths(args))
    output_dir = args.output_dir.resolve() if args.output_dir is not None else args.work_dir.resolve() / "glb"
    manifest = export_catalog_glb(catalogs, output_dir, kind=args.kind, item_id=args.item_id)
    selection = "all components" if args.kind is None else (args.kind if args.item_id is None else f"{args.kind}/{args.item_id}")
...
def _add_catalog_selection(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("kind", nargs="?", choices=SHOWROOM_KINDS, help="optional component kind to select")
    parser.add_argument("item_id", nargs="?", help="optional catalog item id; requires kind")
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Prepare, diagnose, view, and export the FPV component Catalog.")
    subparsers = result.add_subparsers(dest="command", required=True)
    prepare_parser = subparsers.add_parser("prepare", help="create a local showroom Python environment and install dependencies")
    prepare_parser.add_argument("--work-dir", type=Path, default=DEFAULT_WORK_DIR)
    prepare_parser.add_argument("--recreate", action="store_true", help="recreate the managed virtual environment before installing")
    doctor_parser = subparsers.add_parser("doctor", help="validate Catalog loading, MJCF, MuJoCo loading, and GLB export")
    doctor_parser.add_argument("--work-dir", type=Path, default=DEFAULT_WORK_DIR)
    doctor_parser.add_argument("--catalogs", type=Path, action="append", help="catalog directory; repeat to compose public and private catalogs")
    viewer_parser = subparsers.add_parser("open-viewer", help="generate the selected Catalog showroom and open MuJoCo Viewer")
    _add_catalog_selection(viewer_parser)
    viewer_parser.add_argument("--output", type=Path, help="override generated showroom MJCF path")
    glb_parser = subparsers.add_parser("export-glb", help="export selected Catalog parts as browser-ready GLB assets")
    _add_catalog_selection(glb_parser)
    glb_parser.add_argument("--output-dir", type=Path, help="override GLB output directory")
    return result


def _delegate_to_managed_python(raw_argv: list[str], work_dir: Path) -> int:
    python = require_file(managed_python(work_dir), "catalog showroom Python (run prepare first)")
    env = os.environ.copy()
    env[MANAGED_ENV_FLAG] = "1"
    print(f"+ {python} {Path(__file__).resolve()} {' '.join(raw_argv)}", flush=True)
    completed = subprocess.run([str(python), str(Path(__file__).resolve()), *raw_argv], cwd=ROOT, env=env, check=False)
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (CatalogToolError, subprocess.CalledProcessError, RuntimeError, ImportError) as exc:
