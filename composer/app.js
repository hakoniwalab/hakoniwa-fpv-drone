...
const state = { manifest: null, contract: null, components: new Map(), nodes: [], connections: [], selectedNode: null, selectedProvider: null, catalogKind: "all", objects: new Map() };
...
const categoryLabel={all:"All",frame:"Frame",motor:"Motor",propeller:"Propeller",battery:"Battery",camera:"Camera",controller:"Controller",landing_gear:"Landing Gear",accessory:"Accessory"};
function categoryList() {
  const categories=["all",...new Set(state.manifest.items.map((item)=>item.kind))];
  const container=$("categories");
  container.replaceChildren();
  for(const kind of categories) {
    const button=document.createElement("button");
    button.className=`category${state.catalogKind===kind ? " active" : ""}`;
    button.textContent=categoryLabel[kind] || kind.replaceAll("_"," ");
    button.onclick=()=>{ state.catalogKind=kind; categoryList(); cardList(); };
...
function cardList(){
  ...
  const template=$("card");
  const items=state.manifest.items.filter((item)=>state.catalogKind==="all" || item.kind===state.catalogKind);
  for(const item of items){
    ...
    card.addEventListener("dragstart",(event)=>event.dataTransfer.setData("text/plain",JSON.stringify({kind:item.kind,id:item.id})));
...
function assemblyList() {
  ...
  for(const entry of state.nodes) {
    ...
    select.onclick=()=>{ state.selectedNode=entry.id; state.selectedProvider=null; inspector(); assemblyList(); };
