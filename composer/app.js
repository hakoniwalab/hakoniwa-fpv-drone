...
function poseMatrix(pose){ const [r,p,y]=pose.rpy_deg.map((v)=>THREE.MathUtils.degToRad(v)); return new THREE.Matrix4().compose(new THREE.Vector3(...pose.position_m),new THREE.Quaternion().setFromEuler(new THREE.Euler(r,p,y,"XYZ")),new THREE.Vector3(1,1,1)); }
...
function vectorFields(label, value, units, change){ const wrapper=document.createElement("label"); wrapper.className="field"; wrapper.innerHTML=`<span>${label}</span><span class="triplet"></span>`; const names=["X","Y","Z"]; value.forEach((number,index)=>{ const input=document.createElement("input"); input.type="number"; input.step=units==="cm"?"0.1":"1"; input.value=units==="cm"?(number*100).toFixed(2):number.toFixed(1); input.title=names[index]; input.onchange=()=>change(index,Number(input.value)/(units==="cm"?100:1)); wrapper.querySelector(".triplet").append(input); }); return wrapper; }
function inspector(){ const panel=$("selection"); panel.replaceChildren(); const selected=node(state.selectedNode); if(!selected) { panel.className="empty"; panel.textContent="部品またはportを選択してください。"; return; } panel.className=""; const part=component(selected.kind,selected.product); panel.innerHTML=`<strong>${part.name || selected.product}</strong><p class="port">${selected.kind} / ${selected.id}</p>`; const connection=connectionFor(selected.id); if(connection){ const target=state.contract.connection_rules.find((r)=>r.provider_interface===port(node(connection.provider.node),connection.provider.port,"provider").interface && r.consumer_interface===port(selected,connection.consumer.port,"consumer").interface); panel.append(vectorFields("位置 cm",connection.adjustment.position_m,"cm",(i,v)=>{ connection.adjustment.position_m[i]=v; redraw(); })); panel.append(vectorFields("RPY deg",connection.adjustment.rpy_deg,"deg",(i,v)=>{ connection.adjustment.rpy_deg[i]=v; redraw(); })); panel.insertAdjacentHTML("beforeend",`<p class="hint">接続: ${connection.provider.node}.${connection.provider.port}<br>許可: ${target.adjustment.translation_axes.join(",")||"固定"} / ${target.adjustment.rotation_axes.join(",")||"固定"}</p>`); }
